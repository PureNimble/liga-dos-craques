...
O código vive em `src/features/<domínio>/` (auth, games, teams, events, stats, xp, rankings, achievements, challenges, places, profile, settings, notifications, tracking, admin, awards, feedback, health). Cada feature agrupa os seus componentes, schemas Zod e hooks. Convenções dentro de uma feature:
