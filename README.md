...
| `npm run test:db` | Testes de BD/RLS (pgTAP) — requer `supabase start` (Docker) |

### Testes

- **Unitários** (`npm test`): lógica pura — balanceador de equipas, rating, schemas de validação, datas.
- **BD/RLS** (`npm run test:db`): invariantes de segurança em `supabase/tests/*.sql` (pgTAP). Requer Docker + `supabase start`. Corre também em CI se configurado.
...
## Nota de privacidade (perfis)

Na F1, **todos os utilizadores autenticados conseguem ler todos os perfis** (incluindo idade/peso/altura). É uma decisão deliberada: é uma app entre amigos e a plataforma existe precisamente para mostrar atributos e estatísticas de cada jogador. A escrita é sempre **só do dono** (RLS + GRANTs por coluna — o campo `role` nunca é editável pelo próprio, evitando escalada de privilégios).

Se mais tarde quiseres esconder dados sensíveis, o caminho é separar as colunas privadas (`birth_date`, `weight_kg`, `height_cm`) numa tabela `profile_private` com RLS restrita ao dono. Não requer reescrita — só uma migração.
